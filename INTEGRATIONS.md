...
**clearancepath** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `clearancepath` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
clearancepath ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
clearancepath ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
clearancepath ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `clearancepath` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="clearancepath") for rec in records]   # records = your JSON output
