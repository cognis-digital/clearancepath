"""CLEARANCEPATH — JSON rule engine."""
...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CP-SEAD3-001', 'severity': 'high', 'weight': 2.5, 'title': 'OVERDUE_SEAD3_REPORT', 'when': {'reportable_event': True, 'reported_within_days': '^([4-9]|\\d{2,})'}, 'remediation': 'File SEAD-3 report immediately; brief FSO.'}, {'id': 'CP-TRAIN-001', 'severity': 'medium', 'weight': 2.0, 'title': 'TRAINING_OVERDUE', 'when': {'training_overdue': True}, 'remediation': 'Schedule annual security refresher.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="clearance",
                    metadata=item,
                ))
